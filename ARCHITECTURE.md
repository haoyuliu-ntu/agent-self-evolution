# ARCHITECTURE — 自进化 Agent 项目技术方案

## 一、项目定位

本项目是一个**可复现、低成本、强视觉**的 Agent 自进化机制 Demo，参考 Anthropic
Hermes Agent 的 Skill Nudge 机制与 DSPy GEPA 的离线优化思路，在虚构电商客服场景下
演示：
...
| `serve.py` + `index.html` | 步进式 Web  UI | FastAPI + SSE，逐块手动点击推进 |
...
例：数字商品规则与常规退款有根本差异（优先级不同），适合单独成 Skill。
...
3. **触发条件清晰可解释**：开发者能精准看到"因为这 N 条失败，所以做这 M 个改动"
...
修数字商品、block 3 修 VIP 退款... 每块的进化轨迹对应清晰的节拍。
...
开发者看准确率分类表就能判断下一步该做什么：推脱多 → Reviewer 要 create Skill；
...
（比如 promotion 既涉及满减又涉及券），这正是想呈现的"梯度进化"。
...
**本项目缓解**：评估集里 Q6（"退货运费谁出"）特意注明"我是普通用户"，避免用户
...
- 成本：一次完整实验 ~200 次 LLM 调用 ≈ 0.2 元（demo 友好）
- 可复现：规则判定是确定性的
- 透明：规则简单，开发者能理解

**天花板约 90%**。如果需要突破，真实生产会叠加三层：规则做快速筛选 → LLM-as-judge
做复核 → 人工抽查。本项目选规则层，聚焦"Skill 进化机制"的，不跑这个三层栈。
